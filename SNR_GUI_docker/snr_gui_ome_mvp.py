...
st.set_page_config(page_title="SNR OME MVP (segmentation.py)", layout="wide")
st.title("SNR Calculator (OME-TIFF MVP via segmentation.py)")
st.caption(
...
uploaded = st.file_uploader(
    "Upload one or more OME-TIFF files (each file = one ROI)",
...
if not st.button("Run segmentation.py + SNR"):
    st.stop()
...
logs_csv = pd.DataFrame(runtime_logs).to_csv(index=False).encode("utf-8")
...
st.download_button(
    ...
    data=zip_buf,
    file_name="snr_ome_mvp_results.zip",
