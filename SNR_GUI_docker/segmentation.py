...
def load_stardist_model():
    ...
    local_model_dir = os.environ.get("STARDIST_MODEL_DIR", "").strip()

    if local_model_dir:
        model_root = Path(local_model_dir)
        if model_root.exists():
            # Find any StarDist model folder that contains both expected files.
            for weights_file in model_root.rglob("weights_best.h5"):
                model_dir = weights_file.parent
                if not (model_dir / "thresholds.json").exists():
                    continue

                print(
                    ">>> Loading StarDist model from local directory = " + str(model_dir),
                    flush=True,
                )
                return StarDist2D(None, name=model_dir.name, basedir=str(model_dir.parent))

        print(
            ">>> STARDIST_MODEL_DIR set but no local StarDist model found under: " + str(model_root) + ". Falling back to pretrained.",
            flush=True,
        )
...
def upscale_results(df):
    if pipex_scale_factor > 0:
        ...
        show_png = os.path.join(data_folder, "analysis", "segmentation_mask_show.png")
        if os.path.isfile(show_png):
            image = PIL.Image.open(show_png)
            image = image.resize((image.size[0] * pipex_scale_factor, image.size[1] * pipex_scale_factor))
            image.save(show_png)
        show_jpg = os.path.join(data_folder, "analysis", "segmentation_mask_show.jpg")
        if os.path.isfile(show_jpg):
            image = PIL.Image.open(show_jpg)
            image = image.resize((image.size[0] * pipex_scale_factor, image.size[1] * pipex_scale_factor))
            image.save(show_jpg)
...
def fallback_nuclei_segmentation(nuclei_img_orig):
    # Fallback for environments where pretrained StarDist cannot be initialized.
    img = np.asarray(nuclei_img_orig, dtype=np.float32)
    img = np.nan_to_num(img, nan=0.0, posinf=0.0, neginf=0.0)
    img_min = float(np.amin(img))
    img_max = float(np.amax(img))
    if img_max <= img_min:
        return np.zeros_like(img, dtype=np.int32)

    img_n = (img - img_min) / (img_max - img_min)
    try:
        thr = threshold_multiotsu(img_n, 3)[0]
    except Exception:
        thr = float(np.mean(img_n))
    mask = img_n > thr

    approx_area = max(8, int((max(float(nuclei_diameter), 1.0) ** 2) * 0.2))
    mask = remove_small_objects(mask, min_size=approx_area)
    if not np.any(mask):
        return np.zeros_like(img, dtype=np.int32)
...
def cell_segmentation(nuclei_img_orig, membrane_img_orig, custom_img_orig):
    if custom_segmentation == "" or custom_segmentation_type != "nuc":
        #normalizing images
        nuclei_img_min = np.amin(nuclei_img_orig)
        nuclei_img_max = np.amax(nuclei_img_orig)
        nuclei_img_range = nuclei_img_max - nuclei_img_min
        if nuclei_img_range > 0:
            nuclei_img = (nuclei_img_orig - nuclei_img_min) / nuclei_img_range
        else:
            nuclei_img = np.zeros_like(nuclei_img_orig, dtype=np.float32)
        ...
        try:
            ...
            print(">>> Stardist unavailable, using fallback nuclei segmentation", flush=True)
            print(">>> ", e, flush=True)
            sd_labels = fallback_nuclei_segmentation(nuclei_img_orig)
            print(">>> Fallback nuclei segmentation done =", datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S"), flush=True)
    ...
    nuclei_img_show = np.asarray(nuclei_img_orig, dtype=np.float32)
    p1, p99 = np.percentile(nuclei_img_show, [1, 99])
    if p99 <= p1:
        p1 = float(np.min(nuclei_img_show))
        p99 = float(np.max(nuclei_img_show) + 1e-6)
    nuclei_img_show = np.clip((nuclei_img_show - p1) / (p99 - p1), 0, 1)
    mask_show = np.uint8(mark_boundaries(nuclei_img_show, sd_labels_expanded) * 255)
    imsave(os.path.join(data_folder, "analysis", "segmentation_mask_show.png"), mask_show)
    # Backward-compatible output for existing tools.
    imsave(os.path.join(data_folder, "analysis", "segmentation_mask_show.jpg"), mask_show)
    ...
    if np.amax(sd_labels_expanded) <= 255:
        ...
        imsave(os.path.join(data_folder, "analysis", "segmentation_mask.tif"),
       sd_labels_expanded.astype(np.uint32))
...
if __name__ =='__main__':
    ...
    try:
        ...
        print(">>> upscale_results skipped due to error:", e, flush=True)
